...
export class AuthService {
  ...
  async resendVerificationByEmail(email) {
    const user = await this.authRepo.findUserByEmail(email);
    if (!user) {
      throw new ValidationError('Bu e-posta adresine ait hesap bulunamadı');
    }
  ...
  async login({ email, password }) {
    ...
    if (!user.is_verified) {
      ...
      eventEmitter.emit('send-verification-code', { email: user.email, code });
      throw new UnauthorizedError('Lütfen önce e-posta adresinizi doğrulayın');
    }

    if (!user.is_active) {
      throw new UnauthorizedError('Hesap devre dışı bırakıldı');
    }
